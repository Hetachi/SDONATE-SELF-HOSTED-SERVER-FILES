	...
	foreach($results as $key => $value){
		...
		if($results[$key]['usernametype'] === 'Steam ID'){
			...
			if($rowcount1 > 0){
				...
				$steamusername = $results1[0]['username'];
				$usernamecached = TRUE;
			}
							...
							if($totalTransactions > 0){
								foreach($results as $key => $value){
									...
									print(
										...
										'<td><a class="underlined-link" href="dashboard.php?action=users&username=' . htmlspecialchars_decode($results[$key]['purchaser']) . '">' . $results[$key]['purchaser'] . '</a></td>' .
