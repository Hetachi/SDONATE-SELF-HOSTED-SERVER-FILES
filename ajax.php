							...
							if($results[0]['usernametype'] === "Steam"){
								if(!isset($_POST['gift-checkbox']) OR $giftable == 0){
									...
									if(empty($results[0]['steamid'])){
										...
										$steamid32 = "STEAM_1:$authserver:$authid";
										$vars['STEAMID32'] = $steamid32;
										$steamID3 = $authid * 2;
										$steamID3++;
										$vars['STEAMID3'] = "U:1:" . $steamID3;
										$goToNextStage = true;
									}
								} else {
	...
	if($allowedToPurchase === true){
		...
		if($allowedToPurchase === true){
			...
			if($paymentMethod === "paypal"){
				...
				} elseif($result === "nopaypalemail"){
					array_push($pageError, "You haven't set your PayPal email at https://sdonate.com");
				} else {
					$paypalEmail = strtolower($result);
					$sql = $dbcon->prepare("UPDATE settings SET value=:paypalemail WHERE setting='paypalemail'");
					$values = array(':paypalemail' => $result);
					$sql->execute($values);
